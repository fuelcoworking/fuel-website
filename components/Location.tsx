...
export default function Location() {
  ...
  return (
    <section
      id="location"
      className="relative bg-dark px-6 py-16 lg:px-10 lg:py-20 xl:px-12"
    >
      <div
        className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-primary via-amber to-primary"
        aria-hidden
      />
        ...
        <div className="grid grid-cols-1 items-start gap-10 lg:grid-cols-[1fr_1.15fr] lg:gap-16">
          <div>
            ...
            <dl className="mt-8 space-y-5">
              {details.map((item) => (
                <div key={item.label}>
                  <dt className="font-display text-xs font-medium uppercase tracking-wide text-amber">
                    {item.label}
                  </dt>
                  <dd className="font-body mt-1 whitespace-pre-line text-[15px] leading-relaxed text-cream/80 md:text-base">
                    {item.value}
                  </dd>
                </div>
              ))}
            ...
            <a
              ...
              className="font-display mt-8 inline-block rounded-full bg-white px-7 py-3.5 text-[15px] font-bold text-dark transition-opacity hover:opacity-90"
            >
              Get directions
            </a>
          </div>
          ...
          <div className="flex w-full flex-col lg:sticky lg:top-20">
            ...
            <p className="font-body mt-3 text-xs text-cream/50">
