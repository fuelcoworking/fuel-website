...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="w-full rounded-full bg-primary py-3 font-medium text-cream transition-colors hover:bg-orange disabled:opacity-60"
    >
    ...
    <section id="contact" className="bg-cream px-6 py-24">
      <div className="mx-auto max-w-xl text-center">
        ...
        {state.success ? (
          <p
            role="status"
            className="mt-10 rounded-2xl border border-primary/30 bg-white px-6 py-8 text-lg font-medium text-dark"
          >
          ...
          <form action={formAction} className="mt-10 space-y-4 text-left">
            {state.error && (
              <p role="alert" className="rounded-lg bg-primary/10 px-4 py-3 text-sm text-dark">
                {state.error}
              </p>
            )}
