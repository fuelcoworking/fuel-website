...
import { submitFreeDay, type FreeDayState } from "@/app/actions/free-day";

const initialState: FreeDayState = { success: false, error: null };
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Sending…" : "Claim my free day"}
...
export default function FreeDayForm() {
  const [state, formAction] = useFormState(submitFreeDay, initialState);
    ...
    return (
      <div className="rounded-xl border border-primary/20 bg-white p-6 lg:p-8">
        ...
        <p className="font-body mt-3 text-[15px] leading-relaxed text-dark/75 md:text-base">
          We&apos;ll reach out to confirm your visit. When you arrive at FUEL,
          check in at the front desk and we&apos;ll set you up with 24 hours of
          full access to try the space. Your next idea takes off from here.
        </p>
      </div>
    );
  }
    ...
    <form action={formAction} className="rounded-xl border border-dark/10 bg-white p-6 lg:p-8">
      ...
      <div className="mt-5">
        ...
        <textarea
          ...
          name="notes"
          rows={3}
          placeholder="What you do, what you're looking for, questions…"
